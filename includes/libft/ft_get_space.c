...
static int	get_space_util(char **joined, char **strzeros, char *retour)
{
	*joined = ft_strjoin("-", *strzeros);
	...
	if (!*joined)
		return (0);
	*strzeros = *joined;
	*joined = ft_strjoin(retour + 1, *strzeros);
	return (1);
}
...
char	*ft_get_space(int arrondi, char *retour, long long int entier, char c)
{
	...
	if (entier < 0 && (arrondi - ft_strlen(retour) + 1) > 1)
	{
		strzeros = ft_get_str_zeros(arrondi - ft_strlen(retour), c);
		if (strzeros)
		{
			if (!get_space_util(&joined, &strzeros, retour))
				return (0);
		}
		else
			return (0);
	...
	{
		strzeros = ft_get_str_zeros(arrondi - ft_strlen(retour), c);
		joined = ft_strjoin(retour, strzeros);
	}
